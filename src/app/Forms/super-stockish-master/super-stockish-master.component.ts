...
@Component({
  ...
  styleUrls: ['./super-stockish-master.component.css']
})
export class SuperStockishMasterComponent implements OnInit {
  isEdit:boolean = true;
  ROLE_LIST=[];
  SALEROLE_LIST=[];
  WEF_LIST=[];
  ...
  GetUserList() {
    ...
    this.http.postnew(this.url.GETSUPERSTOCKISTMASTERLIST, data).then(
      (res:any)=>{
        console.log("userList",res);//PRODUCTLIST
        this.isLoaded= false;
        this.userList=res.MASTER_DETAILS;
  ...
  onUserSelected(data:any){
    ...
    this.isEdit = false;
    this.loginId = data.SUPERSTOCKIST_CODE;
    this.USER_ID = data.USER_ID;
    this.userName = data.SUPERSTOCKIST_NAME;
  ...
  SaveUserMaster() {
    ...
    let data={
      ...
      "USER_ID":(this.USER_ID == null ? 0 :this.USER_ID),
