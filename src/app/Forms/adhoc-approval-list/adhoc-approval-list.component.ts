...
@Component({
  selector: 'app-adhoc-approval-list',
  templateUrl: './adhoc-approval-list.component.html',
  styleUrls: ['./adhoc-approval-list.component.css']
})
export class AdhocApprovalListComponent implements OnInit {
 ...
 constructor(private authService: AuthService, private url: URLService, private http: HttpService,
  ...
  ngOnInit(): void {
    ...
    this.http.postnew(this.url.GETADHOCSAMPLEREQUISITIONLISTBYUSERID, data).then(
      (res: any) => {
...
GETADHOCSAMPLEREQUISITIONLIST() {
  ...
  GETSMCALCULATEPACKQTYBYPOOLCODE(data,index){
    ...
    this.http.postnew(this.url.GETADHOCSMCALCULATEPACKQTYBYPOOLCODE, data1).then((res: any) => {
