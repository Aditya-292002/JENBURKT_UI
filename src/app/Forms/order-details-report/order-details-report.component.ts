...
@Component({
  ...
  styleUrls: ['./order-details-report.component.css']
})
export class OrderDetailsReportComponent {
  ...
  REPORT_HEADERS:any=[];
   constructor(private AuthService:AuthService,private url:URLService,private http:HttpService,private toastrService:ToastrService,private fileDownloadService: ApiService,private sanitizer: DomSanitizer
    ...
    ngOnInit(): void {
     ...
     this.http.postnew(this.url.GETORDEREPORTMASTERLIST, data).then(
       (res:any)=>{
   ...
   //  console.log('this.AREA_CODE',this.AREA_CODE);
    ...
    //  }
     ...
     this.isLoaded=true;
         this.http.postnew(this.url.GETORDERDETAILSREPORTBYPERIODID, data).then(
       (res:any)=>{
  ...
  exportExcel(){
    this.comman.exportExcelSuperstockist(this.REPORT_DATA)
